...
public class YoloService {
    ...
    private String cfgPath = "D:\\yolo\\yolov4.cfg";

    private String weightsPath;

    private String namesPath = "D:\\yolo\\coco.names";
    ...
    private void init() throws Exception {
        ...
        // 神经网络初始化
        net = readNetFromDarknet(cfgPath);
        // 检查网络是否为空
        if (net.empty()) {
            log.error("神经网络初始化失败");
    ...
    public static void main(String[] args) throws Exception {
        YoloService yoloService = new YoloService();
        yoloService.init();
        Map<String, Object> map = new HashMap<>(8);
        String originalFileName = "D:\\yolo\\1111.jpg";
        // 读取文件到Mat
        Mat src = imread(originalFileName);
        // 执行推理
        MatVector outs = yoloService.doPredict(src);
        // 处理原始的推理结果，
        // 对检测到的每个目标，找出置信度最高的类别作为改目标的类别，
        // 还要找出每个目标的位置，这些信息都保存在ObjectDetectionResult对象中
        List<ObjectDetectionResult> results = yoloService.postprocess(src, outs);
        ...
        // 计算出总耗时，并输出在图片的左上角
        yoloService.printTimeUsed(src);

        // 将每一个被识别的对象在图片框出来，并在框的左上角标注该对象的类别
        yoloService.markEveryDetectObject(src, results);

        // 将添加了标注的图片保持在磁盘上，并将图片信息写入map（给跳转页面使用）
        yoloService.saveMarkedImage(map, src);
    ...
    private void markEveryDetectObject(Mat src, List<ObjectDetectionResult> results) {
        ...
        for (ObjectDetectionResult result : results) {
            ...
            // annotate on image
            rectangle(src, new Point(result.getX(), result.getY()), new Point(result.getX() + result.getWidth(), result.getY() + result.getHeight()), Scalar.MAGENTA, 1, LINE_8, 0);
    ...
    private MatVector doPredict(Mat src) {
        ...
        Mat inputBlob = blobFromImage(src, 1 / 255.0, new Size(width, height), new Scalar(0.0), true, false, CV_32F);
    ...
    private void printTimeUsed(Mat src) {
        ...
        putText(src, String.format("Inference time : %.2f ms", t), new Point(10, 20), FONT_HERSHEY_SIMPLEX, 0.6, new Scalar(255, 0, 0, 0), 1, LINE_AA, false);
    }
    ...
    private List<ObjectDetectionResult> postprocess(Mat frame, MatVector outs) {
        ...
        for (int i = 0; i < indices.limit(); ++i) {
            ...
            detections.add(new ObjectDetectionResult(clsId, names.get(clsId), confidences.get(idx), box.x(), box.y(), box.width(), box.height()));
